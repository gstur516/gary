...
(function () {
	...
	function moveToLevel4(){
		...
		if(GameManager.company.gameLog.length == 0){
			GameManager.company.notifications.push(new Notification("CheatMod", "To continue to the final level, you need to have at least created 1 game"));
			return
		}
		...
		if(GameManager.company.currentLevel != 4){
			...
			if(GameManager.company.staff.length<2){
				...
				var character = new Character({
                    id: GameManager.getGUID(),name: "Dev",dF: 2,
	...
	div.append('<div id="AAAResearch" class="selectorButton whiteButton" onclick="UI.pickCheatClick(this)" style="display:inline-block;position: relative;margin-left:50px;width: 450px">Add AAA games</div>');
	...
	div.append('<div id="setFastEngineDevModeEnabled" class="selectorButton whiteButton" onclick="UI.pickCheatClick(this)" style="margin-left:50px;width: 450px">Activate fast engine development mode</div>');
	...
	div.append('<div style="margin-left:50px;width: 450px">Move through time, only use this for mod testing! Moving back in time can break your save, moving forwards in time works!</div>');
	...
	var setPerfectScoreEnabled = function(){
		if(perfectScores){
			...
			var div = $("#CheatContainer");
			div.find("#setPerfectScoreEnabled").html("Activate always have perfect scores");
			perfectScores = false;
		}else{
			GDT.on(GDT.eventKeys.gameplay.afterGameReview, setPerfectScores);
			var div = $("#CheatContainer");
			div.find("#setPerfectScoreEnabled").html("Deactivate always have perfect scores");
			perfectScores = true;
		}
	}
    ...
    var setFastEngineDevEnabled = function () {
        if (fastEngineDev) {
            var div = $("#CheatContainer");
            div.find("#setFastEngineDevModeEnabled").html("Activate fast engine development mode");
            fastEngineDev = false;
        } else {
            var div = $("#CheatContainer");
            div.find("#setFastEngineDevModeEnabled").html("Deactivate fast engine development mode");
            fastEngineDev = true;
        }
    }
	...
	var generateTechLevelScreen = function () {
		...
		if(typeof game == 'undefined' || game == null){
			var div = $("#cheatmodTechLevels");
			div.append('Requirement: Needs at least 1 released game.');
			return;
		}
		...
		for (var i = 0; i < xpItems.length; i++) {
			...
			div.append(element);
			element.append('<div id="featureTechLvl" class="selectorButton whiteButton" onclick="CheatModSwitchLove.addTechLevel({0})" style="display:inline-block;position: relative;margin: 0px;top:3px;line-height: 20px;height:22px;width: 22px;" >+</div>'.format(i));
	...
	CheatModSwitchLove.addTechLevel = function(i){
	...
	var initTechAndDesignButtons = function(data){
		if(designAndTechAdded == false){
			CheatModSwitchLove.addTechAndDesignPointsButtons();	
